...
for idx = 1:20 
    for e = 1:num
        ...
        x1=[];y1=[];X = {};
        for file = 1:10  
            RealPath{k}=[al{e},ds,name,num2str(file),txt];
            [x,y]=DataRead(RealPath{k});
            x1 = [x1,x];
            y1 = [y1,y];
            results{idx,e,file}=[x' y'];
            allfit=[allfit;[x' y']];
            k = k+1;
        end
    end
    maxfit=max(allfit);
    minfit=min(allfit);
    maxminf{idx}=[maxfit;minfit];
end

for idx = 1:20 
    maxfit=maxminf{idx}(1,:);
    minfit=maxminf{idx}(2,:);
    for e = 1:num
        for file=1:10
            paretofit=results{idx,e,file};
            score = HV1(paretofit,maxminf{idx});
            HVresult(idx,e,file)=score;
        end
        meanhv(idx,e)=mean(HVresult(idx,e,1:file));
    end
end
